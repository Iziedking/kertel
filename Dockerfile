...
# native-build question never arises: Telt's storage is node:sqlite, built
# into Node itself.
#
...
COPY apps/telt/package.json apps/telt/
RUN npm ci
...
COPY --from=build /app/apps/telt/dist apps/telt/dist
...
ENV TELT_DATA_DIR=/data
VOLUME ["/data"]

# Not root. The process needs to write one directory and reach three hosts.
RUN useradd --system --uid 10001 --home /app telt \
    && mkdir -p /data && chown -R telt:telt /app /data
USER telt
...
HEALTHCHECK --interval=60s --timeout=10s --start-period=20s --retries=3 \
  CMD node -e "const{openStore}=require('/app/apps/telt/dist/infra/store.js');" 2>/dev/null \
      || node --input-type=module -e "import('/app/apps/telt/dist/infra/store.js').then(m=>{const s=m.openStore(process.env.TELT_DATA_DIR+'/telt.sqlite');s.safetyState();s.close();}).catch(()=>process.exit(1))"

CMD ["node", "apps/telt/dist/daemon.js"]
