...
| web | Public fixture demo, setup guide and local browser signature verification |
...
Account-wide daily realized loss and aggregate exposure are not available from the current accounting implementation. Direct spot orders retain per-order and balance checks, while discretionary live entries are paused. Do not present configured account-wide caps as enforced.
...
SQLite stores research, decisions, proposals, confirmations, operations, mandates and research-payment bookkeeping. Additive schema migration preserves existing tables. The daemon and public MCP service keep separate named volumes.
...
The public demo is generated by scripts/prove.ts from the fixture runtime, with a fixed clock, recorded responses, a public test key and scripted NO_TRADE decisions. It is not a live AI session or a live payment demonstration.
