 ...
 *
 * **This is the fallback rail.** Telt prefers Binance Agent OS, in
...
export type BinanceClient = {
  ...
  /**
   * Every pair's last twenty-four hours, in one free unauthenticated call.
   *
   * The whole venue, not a page of it: the caller ranks and filters, because a
   * server-side limit would decide for it which movers matter.
   */
  movers(): Promise<Result<readonly Mover[], Refusal>>;
...
export function parseExchangeInfo(
  ...
  if (
    ...
    return refuse(
      "PROVIDER_UNAVAILABLE",
      `Binance's trading rules for ${symbol} are missing a field Telt needs to size an order safely.`,
    );
  }
...
export function createBinanceClient(config: BinanceConfig): BinanceClient {
  ...
  async function call(input: {
    ...
    if (input.signed) {
      if (!credentialed) {
        return refuse(
          "EXECUTION_ADAPTER_UNAVAILABLE",
          "No Binance API key is configured, so Telt can read prices but cannot see the account or place an order.",
        );
      }
    ...
    try {
      ...
      if (input.unknownOnTimeout === true) {
        ...
        return refuse(
          ...
          "Telt sent the order and did not get an answer. It does not know whether the order was placed and will not retry until that is reconciled.",
  ...
  return {
    ...
    async movers(): Promise<Result<readonly Mover[], Refusal>> {
      // No symbol: the whole venue in one call. Free and unauthenticated.
      const result = await call({
        path: "/api/v3/ticker/24hr",
        method: "GET",
        query: {},
        signed: false,
        // The whole venue is a large payload; the default read window is tuned
        // for single-symbol calls.
        timeoutMs: 15_000,
      });
      if (!result.ok) return result;
      if (!Array.isArray(result.value)) {
        return refuse("MARKET_DATA_STALE", "Binance returned no usable 24-hour ticker.");
      }
      const movers = result.value.map(toMover).filter((row): row is Mover => row !== null);
      if (movers.length === 0) {
        return refuse("MARKET_DATA_STALE", "Binance's 24-hour ticker had no readable rows.");
      }
      return ok(movers);
    },
...
/**
 * One row of the 24-hour ticker, or null if it is not usable.
 *
 * Binance returns every listed pair here, including ones with no trades at all,
 * so a row that does not parse is skipped rather than treated as an error: one
 * malformed entry among three thousand must not lose the other 2,999.
 */
export function toMover(row: unknown): Mover | null {
  if (typeof row !== "object" || row === null) return null;
  const entry = row as Record<string, unknown>;

  const symbol = entry["symbol"];
  const last = decimalOrNull(entry["lastPrice"]);
  const volume = decimalOrNull(entry["quoteVolume"]);
  const percent = entry["priceChangePercent"];
  const high = decimalOrNull(entry["highPrice"]);
  const low = decimalOrNull(entry["lowPrice"]);
  ...
  // Percent to basis points, through the fixed-point path rather than
  // parseFloat, so a value like "0.036" cannot arrive as 3.5999999999999996.
  const changeBps = Number(fp.multiply(fp.parse(percent), fp.parse("100")).atoms) /
    10 ** fp.multiply(fp.parse(percent), fp.parse("100")).scale;
...
function decimalOrNull(value: unknown): FixedPoint | null {
  return typeof value === "string" && /^\d+(\.\d+)?$/.test(value) ? fp.parse(value) : null;
}
