 ...
 *    position. Telt sets isolated margin and a low leverage before it opens
...
import type { Refusal, Result, Symbol_ } from "@telt/core/domain";
    ...
    async filters(symbol: Symbol_): Promise<Result<FuturesFilters, Refusal>> {
      ...
      if (
        ...
        return refuse(
          ...
          `Binance's futures rules for ${symbol} are missing a field Telt needs to size a position safely.`,
        );
      }
