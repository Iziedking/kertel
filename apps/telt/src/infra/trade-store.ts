 ...
 * *promise* survives a restart: the user is holding a code that Telt told them
...
-- Signed proofs of research, kept so one can be produced again later.
--
-- The value of an attestation is that somebody can check it long after the
-- conversation that produced it has scrolled away, so it has to outlive the
-- session. Keyed by the provenance digest, which is what the signature commits
-- to, so re-running the same research over the same evidence finds the proof
-- already here rather than signing a second one.
CREATE TABLE IF NOT EXISTS attestations (
  provenance TEXT PRIMARY KEY,
  document TEXT NOT NULL,
  at INTEGER NOT NULL,
  -- The exchange order this evidence justified, once one exists. Filled in on
  -- confirmation, which is what turns "it knew this" into "it did this".
  order_ref TEXT
);
...
-- Positions Telt did not open.
--
-- An order placed through Binance Agent OS lands in the Agentic sub-account
-- and Telt never sees the fill. Without the real entry price it would default
...
export function tradeStore(db: DatabaseSync): TradeStore {
  return {
    ...
    unreconciledOperations() {
      ...
      // land, because open exposure Telt cannot see is exposure it cannot cap.
      const rows = db
        .prepare(
