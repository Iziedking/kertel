/**
 * Carrying Telt's working state between machines.
 *
 * Telt's state lives in a local SQLite file. Open Claude Code on a different
 ...
 * Tuesday says Telt was managing 0.3 ETH. By Thursday the account may hold
...
import type { ExitMandate, LadderRung, MandateId } from "@telt/core/mandates";

import type { BinanceClient } from "./infra/binance.js";
import type { Store } from "./infra/store.js";

const FORMAT = "TELT-STATE-1";
...
export function snapshot(deps: PortableDeps): string {
  ...
  lines.push(`# Telt working state. Hand this to telt_restore on any machine to resume.`);
...
export async function restore(deps: PortableDeps, text: string): Promise<RestoreReport> {
  ...
  if (header === undefined) {
    return {
      ok: false,
      body: `That does not look like a Telt snapshot. It should begin with ${FORMAT}.`,
    };
  }
  ...
  if (deps.ownerHash === null) {
    return { ok: false, body: "Telt has no configured owner, so it will not restore state." };
  }
  ...
  if (writtenBy !== null && writtenBy !== deps.machineId && restored.length > 0) {
    ...
    report.push(
      "two copies of Telt now hold the same plans. An autonomous exit derives its order id",
    );
    report.push(
