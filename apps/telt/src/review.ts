/**
 * What Telt planned, what actually happened, and what to do differently.
 *
 ...
 *
 * **Telt owns the facts.** Every plan it drafted, every exit it fired, every
 ...
 * available to a fresh session on Friday. Telt does not talk to it directly
 ...
 * and Telt duplicating that would break the one property it exists to
 * provide.
 *
 * **The client model is the bridge.** It recalls, hands what it found to
 * `telt_learn`, and stores the digest `telt_review` produces. That is the
 * same division as everywhere else in this product: Telt is deterministic and
...
import { formatInstant, utcDay } from "@telt/core/domain";
import type { Instant, Symbol_ } from "@telt/core/domain";
import { moveBps } from "@telt/core/mandates";
...
export type Lesson = {
  ...
  /** Where it came from: Telt's own review, or recalled from memory. */
...
export function renderReview(deps: ReviewDeps, outcomes: readonly Outcome[]): string {
  ...
  if (stats.closed === 0) {
    return [
      ...
      "",
      "Telt records every exit as it happens — the fill price, the move against entry,",
      "and how much was given back from the peak. Once a few have closed, this becomes",
      "a record you can hold it to.",
    ].join("\n");
  }

  const lines: string[] = ["How Telt has actually done", ""];
...
export function memoryDigest(deps: ReviewDeps, outcomes: readonly Outcome[]): string {
  ...
  const lines: string[] = [];
  lines.push(`${utcDay(now)} — Telt trading record.`);
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "Telt keeps the underlying records locally in its own database; this is the portable summary.",
  );
  return lines.join("\n");
}
