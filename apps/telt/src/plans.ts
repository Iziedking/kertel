 ...
 * from then on Telt manages the position without asking again.
 *
...
import { addSeconds, formatInstant, refuse } from "@telt/core/domain";
import type { Instant, Refusal, SenderIdHash, Symbol_ } from "@telt/core/domain";
import { effectiveStopBps, priceAtBps, validateMandate } from "@telt/core/mandates";
import type { ExitMandate, LadderRung, MandateId, TrailingStop } from "@telt/core/mandates";
import {
...
export type PlanDeps = {
  ...
  /** Null when Telt is not on the Agent OS rail, where futures lives. */
...
export type PlanRequest = {
   ...
   *
   * Null asks Telt to work it out: an open futures position in this symbol
 ...
 * Telt never sees the fill. It can read the resulting balance, but a balance
 ...
 *
 * So adoption is explicit: state what it cost, Telt verifies the position is
...
export async function adoptPosition(
  ...
  if (fp.lessThan(held, quantity)) {
    return fail(
      refuse(
        ...
        `You said ${fp.format(quantity)} ${base} but the account holds ${fp.format(held)}. Telt will not adopt a position it cannot see.`,
      ).error,
    );
  }
  ...
  return {
    ...
    refusalCode: null,
    body: [
      `Telt has taken over ${fp.format(quantity)} ${base}.`,
      "",
      ...
      "current bid. Use telt_plan_exit to hand it a set of rules to manage it by.",
    ].join("\n"),
  };
}
...
export async function planExit(deps: PlanDeps, request: PlanRequest): Promise<PlanOutcome> {
  ...
  if (deps.ownerHash === null) {
    return fail(
      refuse("SENDER_NOT_ALLOWED", "Telt has no configured owner, so it will not arm a plan.").error,
    );
  }
...
function renderPlan(
  ...
  lines.push("");
  lines.push(`Reply  arm ${code}  to hand Telt this position.`);
  ...
  for (const mandate of deps.store.mandates.all()) {
    ...
    if (armed !== null) {
      ...
      return {
        ...
        refusalCode: null,
        body: [
          `Armed. Telt is now managing ${fp.format(armed.quantity)} ${armed.symbol}.`,
          "",
          describePlan(armed),
          "",
          "It will act on these rules on its own. Use telt_positions to see where it stands,",
          "telt_journal to see what it has done, and telt_stop to halt everything.",
        ].join("\n"),
      };
    }
  }
...
export function describePositions(deps: PlanDeps): string {
  ...
  if (mandates.length === 0) {
    return "No exit plans. Use telt_plan_exit to hand Telt a position to manage.";
  }

  const lines: string[] = ["Positions Telt is managing", ""];
...
export function cancelPlan(deps: PlanDeps, id: string): PlanOutcome {
  ...
  return {
    ok: true,
    refusalCode: null,
    body: `Cancelled. Telt is no longer managing ${mandate.symbol}; the position is yours again.`,
  };
}
...
export function describeJournal(deps: PlanDeps, limit: number, withEvidence: boolean): string {
  ...
  const lines: string[] = ["What Telt has been doing", ""];
