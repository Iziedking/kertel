...
import { fixedClock, instant, ok, refuse, seconds } from "@telt/core/domain";
...
function build(options: {
  ...
  const runtime = createRuntime({
    config: loadConfig({
      ...
      ...(options.live === true
        ? { TELT_MODE: "live", TELT_LIVE_EXECUTION: "true" }
        : {}),
    }),
...
describe("preparing an order", () => {
  it("shows the numbers that will actually be sent", async () => {
    ...
    expect(result.body).toContain("Worst fill Telt will accept");
    expect(result.body).toContain("confirm KTL-");
...
describe("an order whose result is unknown", () => {
  const timeout = () =>
    refuse(
      ...
      "Telt sent the order and did not get an answer.",
