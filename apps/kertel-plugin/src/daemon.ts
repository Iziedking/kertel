 ...
 * file: the MCP server is how you talk to Kertel, the daemon is what keeps
 * acting when you close the laptop. SQLite in WAL mode handles both processes.
 *
 * One thing to get right in a container: `KERTEL_DATA_DIR` must point at a
...
async function main(): Promise<void> {
  ...
  log.info("kertel daemon starting", {
    mode: runtime.mode,
  ...
  if (runtime.mode !== "live" || !runtime.config.policy.trading.liveExecutionEnabled) {
    log.warn("no order will be placed", {
      why: "KERTEL_MODE must be live and KERTEL_LIVE_EXECUTION must be true. The monitor will still run and journal what it would have done.",
    });
  }
