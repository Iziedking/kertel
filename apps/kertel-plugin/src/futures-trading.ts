 ...
 *    entire futures wallet backs the position. Kertel sets isolated first and
 ...
 *    the whole margin. Kertel caps it, and the cap is configuration rather than
...
import { formatInstant, refuse } from "@kertel/core/domain";
...
export async function proposeFutures(
  ...
  if (deps.ownerHash === null) {
    return fail(refuse("SENDER_NOT_ALLOWED", "Kertel has no configured owner.").error);
  }
  ...
  if (!isFlat(existing)) {
    return fail(
      refuse(
        "EXCHANGE_REJECTED",
        `There is already a ${positionSide(existing)} position in ${symbol}. Close it before opening another; Kertel will not average into a position you did not plan.`,
      ).error,
    );
  }
...
export async function confirmFutures(deps: FuturesDeps, code: string): Promise<FuturesOutcome> {
  ...
  for (const [codeHash, plan] of pending) {
    ...
    if (deps.mode !== "live" || !deps.liveExecutionEnabled) {
      return fail(
        refuse(
          ...
          `Everything checked out and Kertel stopped at the live write gate. It would have opened ${plan.side === "BUY" ? "a long" : "a short"} of ${fp.format(plan.quantity)} ${plan.symbol} at ${String(plan.leverage)}x isolated.`,
        ).error,
      );
    }
    ...
    lines.push("");
    lines.push("Kertel is watching this position. Use kertel_futures_close to exit,");
    lines.push("or kertel_positions to see where it stands.");
...
export async function closeFutures(
  ...
  if (deps.mode !== "live" || !deps.liveExecutionEnabled) {
    return fail(
      refuse(
        "LIVE_EXECUTION_DISABLED",
        `Kertel would have closed ${fp.format(quantity)} of the ${positionSide(position)} ${symbol} position, but the live write gate is off.`,
      ).error,
    );
  }

  const clientOrderId = clientOrderIdFrom(
    deps.hash(`kertel.futures.close\n${symbol}\n${fp.format(quantity)}\n${String(now)}`),
  );
