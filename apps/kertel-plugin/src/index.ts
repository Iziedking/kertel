 ...
 * is the only seam for custom agent capability. Kertel therefore ships as a
 ...
 * restriction, and Kertel checks the sender again itself. The channel allowlist
 * is a routing convenience, not a security boundary, and neither check is
 * trusted to be the only one.
 *
 * Tool names are all prefixed `kertel_`, because a name that clashes with a
...
export default function kertelPlugin(api: PluginApi): void {
  let runtime: Runtime;
  ...
  try {
    ...
    // other agents, and a misspelled Kertel variable is not their problem.
    // Kertel registers nothing, and the operator sees exactly what to fix.
    const log = createLogger({ level: "error" }).child({ component: "kertel" });
    const message = cause instanceof ConfigError ? cause.message : String(cause);
    log.error("kertel did not start; no tools were registered", { problem: message });
    return;
  }
  ...
  runtime.log.info("kertel tools registered", {
...
export type { KertelConfig } from "./infra/config.js";
