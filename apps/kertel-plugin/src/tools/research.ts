/**
 * `kertel_research` — buy the least evidence that answers the question.
 *
...
import type { ResearchGoal } from "@kertel/core/research";
...
export function researchTool(runtime: Runtime): AgentTool {
  ...
  return {
    name: "kertel_research",
    label: "Kertel research",
    description:
    ...
    async execute(_toolCallId, params) {
      ...
      if (symbol.trim() === "") {
        return textResult("Kertel needs a symbol to research, for example ETHUSDT.", true);
      }
      if (goal === null) {
        ...
        // one that was asked.
        return textResult(
          "Kertel needs to know whether this is a price_check or a trade_thesis. They cost about $0.01 and $0.06.",
          true,
        );
      }
