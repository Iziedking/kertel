 ...
 * `kertel_confirm` is irreversible will call it to "check" something.
 *
...
export function proposeTool(runtime: Runtime): AgentTool {
  ...
  return {
    ...
    async execute(_id, params) {
      ...
      if (symbol === "" || notional === "") {
        return textResult("Kertel needs a symbol and an amount to prepare an order.", true);
      }
      if (side !== "BUY" && side !== "SELL") {
        return textResult("Kertel needs the side to be exactly BUY or SELL.", true);
      }
...
export function confirmTool(runtime: Runtime): AgentTool {
  return {
    ...
    async execute(_id, params) {
      ...
      if (code === "") {
        return textResult("Kertel needs the confirmation code the owner was given.", true);
      }
...
export function reconcileTool(runtime: Runtime): AgentTool {
  return {
    name: "kertel_reconcile",
    label: "Kertel reconcile",
    description:
...
export function stopTool(runtime: Runtime): AgentTool {
  return {
    ...
    label: "Kertel stop",
    description:
      "Engage the kill switch. Kertel immediately refuses all research, proposals and orders until it is " +
...
export function resumeTool(runtime: Runtime): AgentTool {
  return {
    ...
    description:
      ...
      "order or a payment in an unknown state means trading on numbers Kertel cannot vouch for.",
    parameters: Type.Object({}, { additionalProperties: false }),
    ownerOnly: true,
    ...
    async execute() {
      ...
      if (pending.length > 0 || payments.length > 0) {
        // Not a formality. The kill switch was engaged because Kertel lost track
        // of money; releasing it without finding that money just resumes the
        // uncertainty.
        const lines = ["Kertel will not resume yet.", ""];
        ...
        lines.push("");
        lines.push("Run kertel_reconcile first.");
        return textResult(lines.join("\n"), true);
      }

      runtime.store.releaseKillSwitch();
      return textResult("Kertel is running again. Nothing is left unreconciled.");
    },
  };
}
