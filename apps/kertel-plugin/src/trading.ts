...
import { addSeconds, instant, ok, refuse } from "@kertel/core/domain";
import type {
  AccountSnapshot,
  ConfirmationToken,
  ExecutionOperation,
...
import { evaluateConfirmation, evaluateExecution, evaluateProposal } from "@kertel/core/policy";
...
import { hashProposal, idempotencyKeyFor } from "@kertel/core/proposals";
import { DEFAULT_FEE_BPS, sizeFromNotional, slippageBound } from "@kertel/core/proposals";
import { renderProposal, renderRefusalReceipt } from "@kertel/core/receipts";
...
> {
  ...
  if (!deps.binance.credentialed) {
    return refuse(
      ...
      "Kertel has no exchange credentials, so it cannot read your balance and will not propose an order it cannot check you can afford.",
    );
  }
...
export async function propose(
  ...
  if (deps.ownerHash === null) {
    return refusalOf(
      deps,
      refuse("SENDER_NOT_ALLOWED", "Kertel has no configured owner, so it will not prepare an order.")
        .error,
      symbol,
    );
  }
  ...
  if (safety.killSwitchEngaged) {
    return refusalOf(
      ...
      refuse(
        "KILL_SWITCH_ENGAGED",
        `Kertel is stopped: ${safety.killSwitchReason ?? "no reason recorded"}.`,
      ).error,
      symbol,
    );
  }
  if (safety.unreconciledOperations.length > 0) {
    // Open exposure Kertel cannot see is exposure it cannot cap.
    return refusalOf(
      deps,
      refuse(
  ...
  if (!/^\d+(\.\d+)?$/.test(input.notional.trim())) {
    return refusalOf(
      deps,
      refuse("AMOUNT_NOT_UNDERSTOOD", `Kertel could not read ${JSON.stringify(input.notional)} as an amount.`)
        .error,
      symbol,
    );
  }
...
export async function confirm(deps: TradingDeps, code: string): Promise<TradeOutcome> {
  ...
  if (deps.ownerHash === null) {
    return refusalOf(
      deps,
      refuse("SENDER_NOT_ALLOWED", "Kertel has no configured owner, so it will not execute.").error,
      null,
    );
  }
  ...
  if (normalized === null) {
    return refusalOf(
      deps,
      refuse("TOKEN_NOT_FOUND", `${JSON.stringify(code)} is not a Kertel confirmation code.`).error,
      null,
    );
  }
  ...
  if (!executionVerdict.ok) {
    ...
    // who set KERTEL_LIVE_EXECUTION=true while still in fixture mode has the
  ...
  if (deps.mode !== "live") {
    ...
    return refusalOf(
      ...
      refuse(
        ...
        `Everything checked out and Kertel stopped because it is in fixture mode. It would have ${proposal.side === "BUY" ? "bought" : "sold"} ${fp.format(proposal.quantity)} ${symbol} for about ${fp.format(proposal.estimatedNotional)}. Set KERTEL_MODE=live to allow it. The code is still valid.`,
      ).error,
      symbol,
    );
  }
  ...
  lines.push("");
  lines.push(`Kertel ref:  ${operationId}`);
...
export function cancel(deps: TradingDeps): TradeOutcome {
  ...
  if (deps.ownerHash === null) {
    return refusalOf(
      deps,
      refuse("SENDER_NOT_ALLOWED", "Kertel has no configured owner.").error,
      null,
    );
  }
...
/**
 * Ask the exchange what actually happened to the orders Kertel lost track of.
 *
 * This is the other half of never reporting a timeout as a failure. An order
 * whose result is unknown is looked up by the client order id Kertel chose for
...
export async function reconcile(deps: TradingDeps): Promise<string> {
  ...
  lines.push(
    stillOpen
      ? "Some items are still open. Kertel stays stopped until they are resolved."
      : "Everything resolved. Kertel can be resumed.",
  );
