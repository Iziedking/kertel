...
import { instant, ok } from "@kertel/core/domain";
...
/**
 * A market a test can move, and an account that actually shrinks when Kertel
      ...
      if (failNext === "timeout") {
        ...
        return {
          ...
          error: {
            ...
            detail: "Kertel sent the order and did not get an answer.",
          },
        };
      }
...
function build(exchange: ReturnType<typeof fakeExchange>, live = true) {
  ...
  const runtime = createRuntime({
    config: loadConfig({
      ...
      KERTEL_X402_PRIVATE_KEY: `0x${"a".repeat(64)}`,
      KERTEL_BINANCE_API_KEY: "k",
      KERTEL_BINANCE_API_SECRET: "s",
      ...(live ? { KERTEL_MODE: "live", KERTEL_LIVE_EXECUTION: "true" } : {}),
    }),
...
describe("handing Kertel a position", () => {
