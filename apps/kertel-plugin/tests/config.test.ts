...
const BINANCE = { KERTEL_BINANCE_API_KEY: "binance-key", KERTEL_BINANCE_API_SECRET: "binance-secret" };

function env(overrides: Record<string, string | undefined> = {}): Record<string, string | undefined> {
  return { KERTEL_OWNER_WHATSAPP: OWNER, KERTEL_X402_PRIVATE_KEY: KEY, ...overrides };
}
...
describe("an unset variable disables one thing loudly", () => {
  it("runs with nothing configured at all", () => {
    ...
    expect(config.degraded.join(" ")).toContain("KERTEL_OWNER_WHATSAPP");
    expect(config.degraded.join(" ")).toContain("KERTEL_X402_PRIVATE_KEY");
  });

  it("treats an empty string the same as absent", () => {
    const config = loadConfig({ KERTEL_OWNER_WHATSAPP: "   ", KERTEL_X402_PRIVATE_KEY: "" });
    expect(config.ownerWhatsApp).toBeNull();
    expect(config.x402PrivateKey).toBeNull();
  });
...
describe("a variable that is set but wrong is a hard error", () => {
  it("refuses a limit that is not a number", () => {
    expect(() => loadConfig(env({ KERTEL_MAX_TRADE_NOTIONAL: "fifty" }))).toThrow(ConfigError);
  });

  it("refuses a phone number that is not E.164", () => {
    expect(() => loadConfig(env({ KERTEL_OWNER_WHATSAPP: "08067053854" }))).toThrow(ConfigError);
  });

  it("refuses a private key of the wrong shape", () => {
    expect(() => loadConfig(env({ KERTEL_X402_PRIVATE_KEY: "0xnope" }))).toThrow(ConfigError);
  });
  ...
  it("refuses an unknown mode rather than falling back to fixture", () => {
    ...
    expect(() => loadConfig(env({ KERTEL_MODE: "production" }))).toThrow(ConfigError);
  });

  it("refuses an unknown rail preference", () => {
    expect(() => loadConfig(env({ KERTEL_X402_RAIL: "bsc" }))).toThrow(ConfigError);
  });

  it("names the variable in the message, so the operator knows what to fix", () => {
    try {
      loadConfig(env({ KERTEL_MAX_SLIPPAGE_BPS: "half a percent" }));
      expect.unreachable("should have thrown");
    } catch (cause) {
      expect((cause as Error).message).toContain("KERTEL_MAX_SLIPPAGE_BPS");
    }
  });
  ...
  it("refuses limits that cannot be enforced together", () => {
    ...
    expect(() =>
      loadConfig(env({ KERTEL_X402_MAX_PER_CALL_USDC: "5.00", KERTEL_X402_MAX_PER_RUN_USDC: "0.10" })),
    ).toThrow(ConfigError);
  });
});
...
describe("the live write gate", () => {
  it("is off when unset, and when explicitly false", () => {
    expect(loadConfig(env({ KERTEL_MODE: "live" })).policy.trading.liveExecutionEnabled).toBe(false);
    expect(
      loadConfig(env({ KERTEL_LIVE_EXECUTION: "false", KERTEL_MODE: "live" })).policy.trading
  ...
  it("rejects every near-miss outright, because none of them is permission to trade", () => {
    ...
    for (const value of ["yes", "1", "TRUE", "True", "on", "maybe"]) {
      expect(() => loadConfig(env({ KERTEL_LIVE_EXECUTION: value })), value).toThrow(ConfigError);
    }
  });

  it("turns on only for the exact word", () => {
    const config = loadConfig(env({ KERTEL_LIVE_EXECUTION: "true", KERTEL_MODE: "live" }));
...
describe("live mode is a claim that has to be backed", () => {
  it("stays in fixture mode when live is asked for with no wallet", () => {
    const config = loadConfig({ KERTEL_MODE: "live", KERTEL_OWNER_WHATSAPP: OWNER });
    expect(config.mode).toBe("fixture");
  ...
  it("stays in fixture mode when live is asked for with no owner", () => {
    const config = loadConfig({ KERTEL_MODE: "live", KERTEL_X402_PRIVATE_KEY: KEY });
    expect(config.mode).toBe("fixture");
  });

  it("goes live when everything live mode needs is present", () => {
    const config = loadConfig(env({ KERTEL_MODE: "live", ...BINANCE }));
  ...
  it("still goes live without exchange credentials, but says trading is off", () => {
    ...
    const config = loadConfig(env({ KERTEL_MODE: "live" }));
    expect(config.mode).toBe("live");
  ...
  it("says plainly that the execution flag does nothing in fixture mode", () => {
    const config = loadConfig({ KERTEL_LIVE_EXECUTION: "true" });
    expect(config.degraded.join(" ")).toContain("no effect while Kertel is in fixture mode");
  });
});

describe("exchange credentials", () => {
  it("refuses a key without its secret, and a secret without its key", () => {
    expect(() => loadConfig(env({ KERTEL_BINANCE_API_KEY: "k" }))).toThrow(ConfigError);
    expect(() => loadConfig(env({ KERTEL_BINANCE_API_SECRET: "s" }))).toThrow(ConfigError);
  });
  ...
  it("refuses to let the research wallet and the trading authority share one secret", () => {
    ...
    expect(() =>
      loadConfig(env({ KERTEL_BINANCE_API_KEY: KEY, KERTEL_BINANCE_API_SECRET: "s" })),
    ).toThrow(ConfigError);
  });
...
describe("the sender salt", () => {
  it("differs per owner, so two deployments cannot be cross-referenced by hash", () => {
    const first = loadConfig(env({ KERTEL_OWNER_WHATSAPP: "+2348067053854" }));
    const second = loadConfig(env({ KERTEL_OWNER_WHATSAPP: "+2348130118673" }));
