...
// Telt runs its suite against the TypeScript sources, not a build output, so
// a failing test points at the file you edit. Fixture mode is the default and
// no test is allowed to reach the network.
export default defineConfig({
  // The workspace packages export two faces: `telt-source` points at the
  ...
  resolve: { conditions: ["telt-source"] },
  ssr: { resolve: { conditions: ["telt-source"] } },
  test: {
